...
cDOMAdapter.prototype.getElementById	= function(oNode, sId) {
    ...
    return cDOMAdapter.traverse(this, this.getProperty(oNode, "documentElement"), function(oDOMAdapter, oNode) {
...
cDOMAdapter.traverse = function(oDOMAdapter, oParent, fTest) {
    for (var oNode = oParent, oChild; oNode; oNode = oDOMAdapter.getProperty(oNode, "nextSibling")) {
        if (oDOMAdapter.getProperty(oNode, "nodeType") == 1) {
            if (fTest(oDOMAdapter, oNode))
                return oNode;
            if (oChild = cDOMAdapter.traverse(oDOMAdapter, oDOMAdapter.getProperty(oNode, "firstChild"), fTest))
                return oChild;
        }
    }
    return null;
