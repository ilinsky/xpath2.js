...
cDOMAdapter.prototype.getElementsByTagNameNS	= function(oNode, sNameSpaceURI, sLocalName) {
    ...
    (function(oNode) {
        // FIXME: Change from using childNodes property to nextSibling
        for (var nIndex = 0, oChild, oChildren = this.getProperty(oNode, "childNodes"); oChild = oChildren[nIndex]; nIndex++)
            if (this.getProperty(oChild, "nodeType") == 1) {	// cNode.ELEMENT_NODE
                ...
                if (this.getProperty(oChild, "firstChild"))
